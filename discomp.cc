...
int main_discomp(int argc, char ** argv)
{
    ...
    char c;
    while ((c = getopt(argc, argv, "l:e:q:")) >= 0)
    {
        switch(c)
        {
    ...
    NucleotideStats global_params;
    global_params.initialize(jpd_data_params_file);
    ...
    default_model.model_params = & global_params;
    ...
    while (! feof(pileup_input_fh))
    {
        ...
        for (pit = pileup_lines.begin(); pit != pileup_lines.end(); ++pit)
        {
            ...
            global_params.pack(& locus.counts);

            //set default model posterior
            default_model.locus_data = & locus.counts;
            ...
            fprintf(posterior_output_fh, "%s\t%s\t%i\t%c\t%Zu\t%Zu\t%Zu", 
